"""
CUDA_VISIBLE_DEVICES=0 python scripts/analyze_voxel_hotspots_glb_feat.py \
  --dataset-root datasets/waymo \
  --output-dir output_voxel_hotspots_glb_feat \
  --camera-name FRONT \
  --voxel-sizes 0.2 0.4 \
  --max-scenes 4 \
...
CUDA_VISIBLE_DEVICES=0 python scripts/analyze_voxel_hotspots_glb_feat.py \
  --dataset-root datasets/waymo \
  --output-dir output_voxel_hotspots_glb_feat \
  --scene-names 10327752107000040525_1120_000_1140_000 \
  --voxel-sizes 0.2 0.4 \
  --max-views-per-scene 20 \
  --model-id depth-anything/DA3NESTED-GIANT-LARGE \
  --device cuda \
...
def _choose_feat_tokens(raw_feats: Any, feat_mode: str, device: torch.device) -> torch.Tensor:
    ...
    return feat_tokens[0].to(device)
...
def _compute_feature_metrics(
    ...
    feat_mode: str = "last2_avg",
    feat_dim_out: Optional[int] = 64,
    patch_size: int = 14,
    ...
    if raw_feats is None:
        return {}
    if device is None:
        device = voxel_point_counts.device
    ...
    num_voxels = int(voxel_point_counts.shape[0])
    num_points = int(view_ids.shape[0])
    ...
    feat_mean_norm = _normalize_rows(feat_mean)
    ...
    for start in range(0, num_points, feature_chunk_size):
        ...
        cos = (_normalize_rows(point_feat_chunk) * mean_chunk_norm).sum(dim=1)
    ...
    return {
        "feat_l2_mean": voxel_feat_l2_mean,
        "feat_cos_mean": voxel_feat_cos_mean,
        "feature_dim": torch.tensor(float(feat_dim), device=device),
    }
...
def analyze_scene(
    ...
    for voxel_size in voxel_sizes:
        ...
        low_feat_cos_mask = None
        low_feat_cos_mask_np = np.zeros(num_voxels, dtype=bool)

        if not skip_feature_metrics:
            feat_metrics = _compute_feature_metrics(
                raw_feats=getattr(prediction, "raw_feats", None),
                depth_hw=tuple(depth.shape[-2:]),
                view_ids=view_ids,
                ys=ys,
                xs=xs,
                inverse_indices=inverse_indices,
                voxel_point_counts=counts.long(),
                feat_mode=feat_mode,
            ...
            if "feat_cos_mean" in feat_metrics:
                feat_cos_mean = feat_metrics["feat_cos_mean"]
                feat_l2_mean = feat_metrics["feat_l2_mean"]
                feature_dim = float(feat_metrics["feature_dim"].item())

                low_feat_cos_idx = torch.topk(feat_cos_mean, k=k, largest=False).indices
                low_feat_cos_mask = torch.zeros(num_voxels, device=device, dtype=torch.bool)
                low_feat_cos_mask[low_feat_cos_idx] = True
                low_feat_cos_mask_np = low_feat_cos_mask.detach().cpu().numpy()
        ...
        if low_feat_cos_mask is not None:
            low_feat_overlay = np.concatenate([point_colors_np.copy(), np.full((point_colors_np.shape[0], 1), 255, dtype=np.uint8)], axis=1)
            low_feat_overlay[low_feat_cos_mask_np[inverse_np]] = np.array([0, 255, 0, 255], dtype=np.uint8)
        else:
            low_feat_overlay = np.concatenate([point_colors_np.copy(), np.full((point_colors_np.shape[0], 1), 255, dtype=np.uint8)], axis=1)

        # original point cloud
        export_point_cloud(world_points_np, np.concatenate([point_colors_np, np.full((point_colors_np.shape[0], 1), 255, dtype=np.uint8)], axis=1), os.path.join(voxel_dir, "original_points.ply"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, np.concatenate([point_colors_np, np.full((point_colors_np.shape[0], 1), 255, dtype=np.uint8)], axis=1), os.path.join(voxel_dir, "original_points.glb"), glb_transform=glb_transform)

        # point cloud overlays at point level
        export_point_cloud(world_points_np, overlay_colors["std"], os.path.join(voxel_dir, "original_points_top_color_std_overlay.ply"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, overlay_colors["std"], os.path.join(voxel_dir, "original_points_top_color_std_overlay.glb"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, overlay_colors["l2"], os.path.join(voxel_dir, "original_points_top_color_l2_overlay.ply"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, overlay_colors["l2"], os.path.join(voxel_dir, "original_points_top_color_l2_overlay.glb"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, overlay_colors["combined"], os.path.join(voxel_dir, "original_points_hotspots_overlay.ply"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, overlay_colors["combined"], os.path.join(voxel_dir, "original_points_hotspots_overlay.glb"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, low_feat_overlay, os.path.join(voxel_dir, "original_points_low_feat_cos_overlay.ply"), glb_transform=glb_transform)
        export_point_cloud(world_points_np, low_feat_overlay, os.path.join(voxel_dir, "original_points_low_feat_cos_overlay.glb"), glb_transform=glb_transform)

        # voxel-center clouds
        export_point_cloud(voxel_centers_np, color_mean_rgba, os.path.join(voxel_dir, "voxel_centers_mean_color.ply"), glb_transform=glb_transform)
        export_point_cloud(voxel_centers_np, color_mean_rgba, os.path.join(voxel_dir, "voxel_centers_mean_color.glb"), glb_transform=glb_transform)

        if top_std_mask_np.any():
            export_point_cloud(voxel_centers_np[top_std_mask_np], np.tile(np.array([[255, 0, 0, 255]], dtype=np.uint8), (int(top_std_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_color_std_centers.ply"), glb_transform=glb_transform)
            export_point_cloud(voxel_centers_np[top_std_mask_np], np.tile(np.array([[255, 0, 0, 255]], dtype=np.uint8), (int(top_std_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_color_std_centers.glb"), glb_transform=glb_transform)
        if top_l2_mask_np.any():
            export_point_cloud(voxel_centers_np[top_l2_mask_np], np.tile(np.array([[0, 80, 255, 255]], dtype=np.uint8), (int(top_l2_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_color_l2_centers.ply"), glb_transform=glb_transform)
            export_point_cloud(voxel_centers_np[top_l2_mask_np], np.tile(np.array([[0, 80, 255, 255]], dtype=np.uint8), (int(top_l2_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_color_l2_centers.glb"), glb_transform=glb_transform)
        if inter_mask_np.any():
            export_point_cloud(voxel_centers_np[inter_mask_np], np.tile(np.array([[255, 220, 0, 255]], dtype=np.uint8), (int(inter_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_intersection_centers.ply"), glb_transform=glb_transform)
            export_point_cloud(voxel_centers_np[inter_mask_np], np.tile(np.array([[255, 220, 0, 255]], dtype=np.uint8), (int(inter_mask_np.sum()), 1)), os.path.join(voxel_dir, "top_intersection_centers.glb"), glb_transform=glb_transform)
        if low_feat_cos_mask_np.any():
            export_point_cloud(voxel_centers_np[low_feat_cos_mask_np], np.tile(np.array([[0, 255, 0, 255]], dtype=np.uint8), (int(low_feat_cos_mask_np.sum()), 1)), os.path.join(voxel_dir, "low_feat_cos_centers.ply"), glb_transform=glb_transform)
            export_point_cloud(voxel_centers_np[low_feat_cos_mask_np], np.tile(np.array([[0, 255, 0, 255]], dtype=np.uint8), (int(low_feat_cos_mask_np.sum()), 1)), os.path.join(voxel_dir, "low_feat_cos_centers.glb"), glb_transform=glb_transform)
        ...
        np.savez_compressed(
            ...
            low_feat_cos_mask=low_feat_cos_mask_np,
            feat_cos_mean=feat_cos_mean.detach().cpu().numpy() if feat_cos_mean is not None else np.array([]),
            feat_l2_mean=feat_l2_mean.detach().cpu().numpy() if feat_l2_mean is not None else np.array([]),
        )
        ...
        row = {
            ...
            "num_low_feat_cos_voxels": int(low_feat_cos_mask.sum().item()) if low_feat_cos_mask is not None else 0,
            "feature_dim": feature_dim,
            **_quantile_dict(color_std, "color_std"),
            **_quantile_dict(color_l2_mean, "color_l2_mean"),
            **_quantile_dict(depth_range, "depth_range"),
            **(_quantile_dict(feat_cos_mean, "feat_cos_mean") if feat_cos_mean is not None else {}),
            **(_quantile_dict(feat_l2_mean, "feat_l2_mean") if feat_l2_mean is not None else {}),
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--feat-dim-out", type=int, default=64)
    parser.add_argument("--patch-size", type=int, default=14)
    parser.add_argument("--feature-chunk-size", type=int, default=10000)
...
def main() -> None:
    ...
    for scene in scenes:
        scene_rows = analyze_scene(
            ...
            glb_transform=args.glb_transform,
        )
